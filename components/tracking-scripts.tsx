...
export default function TrackingScripts() {
  ...
  useEffect(() => { fetch('/api/site/tracking').then((response) => response.json()).then((data) => { if (data?.enabled) setConfig(data); }).catch(() => undefined); const readConsent = () => { try { setConsent(localStorage.getItem('olhonodoc_tracking_consent') === 'yes'); } catch { setConsent(false); } }; readConsent(); window.addEventListener('olhonodoc-consent', readConsent); return () => window.removeEventListener('olhonodoc-consent', readConsent); }, []);
