...
export async function GET() {
  ...
  try {
    ...
    const customers = await prisma.customer.findMany({ select: { id: true, userId: true, name: true, email: true, createdAt: true }, orderBy: { createdAt: 'desc' }, take: 500 });
    ...
    for (const customer of customers) byEmail.set(customer.email.toLowerCase(), { id: customer.userId || customer.id, name: customer.name, email: customer.email, createdAt: customer.createdAt, orders: [] });
