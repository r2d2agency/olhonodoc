...
export async function GET() {
  ...
  return NextResponse.json({ data: orders.map((order) => ({ id: order.id, plate: order.plate, status: order.status, createdAt: order.createdAt, totalCents: order.totalCents ?? order.amountCents, discountCents: order.discountCents, product: order.product, query: order.query })) });
}
